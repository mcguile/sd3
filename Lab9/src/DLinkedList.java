...
public class DLinkedList {
	...
	public  void insertionSort(){
		...
		while (current != null){
			for (Node prev = current; prev != null && prev.prevNode != null; prev = prev.prevNode) {
				if (prev.value < prev.prevNode.value) {
					if (prev==current) {
		                current=prev.prevNode;
		            }
					insertAfter(prev, prev.prevNode.value);
					delete(prev.prevNode);
				}
			}
